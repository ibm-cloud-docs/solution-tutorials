...
{:shortdesc}
This tutorial walks you through provisioning infrastructure and installing software using Terraform and Ansible on a {{site.data.keyword.vpc_full}} (VPC) virtual server instance (VSI).  It builds on the same infrastructure described in the following two tutorials:
...
- [Securely access remote instances with a bastion host](https://{DomainName}/docs/tutorials?topic=solution-tutorials-vpc-secure-management-bastion-server) for secured maintenance of the servers using a bastion host which acts as a `jump` server it comes with a maintenance security group that should be applied when installing software.
...
1. Optionally automates the procedures using CLI, Terraform or Ansible
...
- On the file system of the provisioning system, the workstation in the architecture diagram above
...
- A file is copied from the file system of the workstation to the public VSI and executed
- Tests are run to verify correct installation
...
## Upload and execute
There may be data and software that is available on the filesystem of your on premise system or CI/CD pipeline that needs to be uploaded to the VSI and then executed.  To demonstrate a script will be uploaded from the filesystem of your workstation. The execution of the script will wait for the index html file to exist indicating that nginx has been installed.  It will then create a file, testupload.html, containing the string `hi`. 
...
The various mechanisms for upload and excution will be discussed below.
...
Instead fill the **User data** text box with the contents found here: [cloud-config.yaml](https://github.com/IBM-Cloud/vpc-tutorials/blob/master/vpc-app-deploy/shared/cloud-config.yaml) (.../vpc-app-deploy/shared/cloud-config.yaml)
...
- to ssh to a VSI ingress over tcp port 22 is required
- to install software egress to port 53 (DNS), 80 and 443 are typically required
...
1. Clone the [git repository](https://github.com/IBM-Cloud/vpc-tutorials.git)
1. cd ../vpc-app-deploy
...
`TF_VAR_ssh_key_name` is the name of the VPC SSH public key in the cloud.  This is the public key that will be loaded into the instance to provide secure ssh access via the private key on your workstation.  Use the cli to verify it exists:
```
ibmcloud is keys
```
{:pre}
...
TODO FIX SSH KEY LINK

## CLI and scripting
### Provisiong resources
[Private and public subnets in a Virtual Private Cloud](https://{DomainName}/docs/tutorials?topic=solution-tutorials-vpc-public-app-private-backend) also contains some handy shell scripts that do everything in the tutorial.
...
- [Private and public subnets in a Virtual Private Cloud](https://{DomainName}/docs/tutorials?topic=solution-tutorials-vpc-public-app-private-backend)
- [Securely access remote instances with a bastion host](https://{DomainName}/docs/tutorials?topic=solution-tutorials-vpc-secure-management-bastion-server)

Makefile:
```
cli_apply: check_TF_VARs
	../vpc-public-app-private-backend/vpc-pubpriv-create-with-bastion.sh us-south-1 $(TF_VAR_ssh_key_name) $(PREFIX) $(TF_VAR_resource_group_name) resources.sh @shared/cloud-config.yaml @shared/cloud-config.yaml $(IMAGE)
```
{:pre}
...
- `default` is the name of the resource group
- `resources.sh` is the output of a successful build
- `shared/cloud-config.yaml1 is the cloud-init file
...
It can be educational to execute this script with `bash -x` to appreciate the cli steps that are required to provision cloud resources: vpc, security group, subnets, instances, etc
...
The backend is handled similarly
{:pre}
...
See troubleshoot above if there are problems it can take a few minutes for software to be installed so be patient.

### Update and analysis
To update the system software from the mirrors create a shell script and scp to the VSIs and execute it.  Similarly new versions of on premises files can be deployed.  Extend the tests to verify correct results.

The monotony and chances for error when managing resources in the console UI has lead to scripting.  Integrating scripting into your CI/CD pipeline can also be error prone since the scripts either do not consider the current state of the resources or do so in an ad hoc way.  This has led to other kinds of orchestration systems.
Two of them: terraform and ansible, are discussed below.

## Terraform
### Inroduction
{: #section_two}
Install terraform and the terraform IBM provider on your workstation.
See, Automating cloud resource provisioning with Terraform [Getting started tutorial](https://{DomainName}/docs/terraform).

TODO FIX TERRAFORM LINK

If you are new to terraform or the IBM VPC resource model it may be helpful
to start with 
[main.tf](https://github.com/IBM-Cloud/vpc-tutorials/blob/master/vpc-app-deploy/tfinstance/main.tf) (.../vpc-app-deploy/tfinstance/main.tf). 
It is a self contained example that creates the recources required to create a VSI that can be reached via ssh from your workstation.
It utilizes the environment variables `TF_VAR_ibmcloud_api_key` and `TF_VAR_ssh_key_name` initialized earlier:
...
terraform apply
terraform output
# notice the ssh command in the output.  try it out
terraform destroy
```
{:pre}

### Provisiong resources
Back to the example this tutorial has been demonstrating.  The terraform configuration creates all the resources in the [Private and public subnets in a Virtual Private Cloud](https://{DomainName}/docs/tutorials?topic=solution-tutorials-vpc-public-app-private-backend) tutorial.  Notice that the the previous two tutorials contribute terraform modules that are referenced by [main.tf](https://github.com/IBM-Cloud/vpc-tutorials/blob/master/vpc-app-deploy/tf/main.tf) (.../vpc-app-deploy/tf/main.tf)
...
- backend_pgw - a public gateway can be connected to the backend subnet.  The frontend has a floating ip connected which provides both a public IP and gateway to the internet.  This is going to allow open internet access for software installation unless the security groups preclude access.  The backend does not have have access to the internet unless backend_pgw is true.  A test will verify by checking the value of htp://HOST:/index.html which will be either ISOLATED (backend) or INTERNET (frontend).  
- frontend_user_data, backend_user_data - cloud-init contents that is executed at instance provision time.

A snippet shows the frontend instance provision of the cloud-init file using the ${var.frontend_user_data} you can find the source in [main.tf](https://github.com/IBM-Cloud/vpc-tutorials/blob/master/vpc-public-app-private-backend/tfmodule/main.tf) (.../vpc-public-app-private-backend/tfmodule/main.tf)
...
## Upload and execute
All terraform resources can have associated provisioners.  The null resource does not provision a cloud resource but can be used to copy the uploaded.sh file and then execute it.  Terraform has some special connection variables to use the bastion.  In [main.tf](https://github.com/IBM-Cloud/vpc-tutorials/blob/master/vpc-app-deploy/tf/main.tf) (.../vpc-app-deploy/tf/main.tf)
```
...
```
# Optionally install ansible in a virtual env if it is not available on your workstation.
