...
3. From the left pane of Android Studio or from the cloned folder, Open the `config.xml` file under the path `app/src/main/res/values` to see the placeholders(`ASSISTANT_ID_HERE`) for service credentials. Enter the service credentials (you saved earlier) in their respective placeholders and save the file. _The ASSISTANT URL is from the {{site.data.keyword.conversationshort}} service credentials you noted while provisioning the service. NOT the workspace URL._
