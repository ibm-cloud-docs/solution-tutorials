...
2. Deploy the VSIs with private network interfaces only. Under the **Uplink Port Speeds** section of the VSI order form, select `Private Only` as in the [Isolate workloads with a secure private network]( https://console.bluemix.net/docs/tutorials/secure-network-enclosure.html#isolate-workloads-with-a-secure-private-network) tutorial.
