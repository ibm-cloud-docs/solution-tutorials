...
Refer to the documentation of services to understand how a service is mapping IAM and Cloud Foundry roles to specific actions. See for example [how the {{site.data.keyword.mon_full_notm}} service maps IAM roles to actions](https://{DomainName}/docs/Monitoring-with-Sysdig?topic=Sysdig-iam#iam).
...
* [Grant permissions to a user to view logs in LogDNA](/docs/Log-Analysis-with-LogDNA?topic=LogDNA-work_iam)
* [Grant permissions to a user to view metrics in Sysdig](/docs/Monitoring-with-Sysdig?topic=Sysdig-iam)
