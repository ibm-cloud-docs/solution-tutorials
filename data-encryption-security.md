...
The {{site.data.keyword.cloudaccesstrailshort}} service records user-initiated activities that change the state of a service in the {{site.data.keyword.BluSoftlayer_notm}}. At the end of this tutorial, you will review the events generated by going over the tutorial steps.
...
1. Copy `template.env` to `.env`:
   ...
   ```
   {: codeblock}
1. Edit `.env` and fill in the blanks with values from:
   * the service endpoint and credentials created for **secure-file-storage-cos**,
