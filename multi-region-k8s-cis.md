...
2. Copy and paste the output to set the KUBECONFIG environment variable.
    ...
    ```
    {: pre}
3. Check cluster region and specify it via `ibmcloud cs region-set` when needed
...
1. Login to IBM Container Registry CLI
    ...
    ```
    {: pre}
2. Check Container Registry Region and ensure it is the same as the one of cluster in previous step via IBM Cloud CLI. If not matching, specify Container Registry region via `ibmcloud cr region-set`. Take note for the **region** which will be used for following steps.
...
4. Build and push Docker image
    ...
    * Push docker image built above to Container Registry. It will be referred by containerized application in pods. Be sure to login CR before push via `ibmcloud cr login`.
        ```bash
