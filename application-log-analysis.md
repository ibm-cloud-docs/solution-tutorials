...
Next, in order to work with application logs, you need to generate them. The application push process above already generated many log entries. Because applications deployed in {{site.data.keyword.Bluemix_notm}} are automatically linked with the {{site.data.keyword.loganalysisshort}} service, nothing needs to be done from our side.
