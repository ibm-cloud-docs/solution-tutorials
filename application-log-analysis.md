...
## Optional: Add Availability Monitoring
In the following, we are going to add Availability Monitoring to the application. The service regularly checks the availability and response time of the app. It can be configured to raise alerts of different severity when thresholds are passed.
1. Add the Availability Monitoring by accessing the dashboard in the [IBM Cloud console](https://console.bluemix.net) and then either provision it as a new service in your organization and space, or by clicking on the application to open the details page and then clicking on **Monitoring**. Use the approach via the application details. The following shows that no tests have been run, but one default test is present.   
