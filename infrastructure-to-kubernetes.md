...
This tutorial walks you through the process of moving a VM based application to IBM Cloud Kubernetes cluster. You will learn how to take an existing application (JPetStore) that runs on WebSphere, containerize it and move it to Kubernetes, and then extend it using other IBM services. Moving existing applications to the container and Kubernetes world can be different from an application to allocation. However, the process is the same. In this tutorial guide, you will learn the process and what to prepare for when moving to Kubernetes.

There are two options when moving applications to Kubernetes, these are: 

- Containerize a small portion of the application while keeping the core application as it is. For some large monolith applications, you may want to only move a portion of the application to the Cloud and leave the core application in your current data center. This means that you will take a small portion of the application and run it as microservice in the cloud using Kubernetes for the deployment. You will repeat that process by keep breaking the monolith application into many microservices and run them all inside a Cluster. 
- Containerize the complete application and deploy it on a Kubernetes cluster. With this approach, you will containerize the complete application, this means building and pushing the Docker container images to the Cloud, and run them inside a Kubernetes cluster. **In this tutorial, you will learn how to tackle this approach.** You learn the process of moving an existing application (JPetStore) to Kubernetes and learn see how it can be then extended very easily.
...
The following diagram outlines the system's high-level architecture. For this example, an existing [Java PetStore application](https://github.ibm.com/ibmcloud/ModernizeDemo) has been selected. The Java PetStore application runs on a WebSphere server with a MySQL database. Let's look at how the existing application was deployed, and it's component. 
...
- WebSphere servers to host the application.
