...
**ToDo**: explain again if data is saved locally in a pod then it will be lost if the pod is removed. 
...
- One way to access the storage is to mount the storage to a pod and copy the files. If you need copy files before the application uses the storage volume. 
- To view the files, run the exec command and run a shell to view the file directory. 
...
Now that you understand the process of moving VM based applications to Kubernetes, next you should run the JPetStore demo to put all of above steps in practice. 
