...
* Obtain APNs (notification provider) credentials and configure Push Notifications service instance.
...
Click on `Download Code` to download the scaffolded code to Mac. In the next step, you will obtain Apple Push Notification Service (APNs) credentials.
...
2. Along with BMSCore, Mobile Analytics is already imported into the project with 
 ...
 ``` 
  import BMSAnalytics
 ```
3. Analytics initialization code is already included as shown below 
...
4. Gathering usage analytics and using logger - Navigate to `ViewController.swift` to see the below code.
