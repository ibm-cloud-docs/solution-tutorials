...
services: containers, openshift, cloud-object-storage, atracker, Registry, secrets-manager, appid, Cloudant, key-protect, cloud-logs, cis
account-plan: paid
completion-time: 2h
...
{: toc-services="containers, openshift, cloud-object-storage, atracker, Registry, secrets-manager, appid, Cloudant, key-protect, cloud-logs, cis"}
...
7. Application management activities are logged by {{site.data.keyword.atracker_full_notm}} and routed to {{site.data.keyword.logs_full_notm}} for analysis.
...
A minimal cluster with one (1) zone, one (1) worker node and the smallest available size (**Flavor**) is sufficient for this tutorial. To create your {{site.data.keyword.containerfull_notm}} cluster, follow the steps for either [Creating VPC clusters](/docs/containers?topic=containers-cluster-create-vpc-gen2&interface=ui) or [Creating classic clusters](/docs/containers?topic=containers-cluster-create-classic). To create your {{site.data.keyword.openshiftlong_notm}} cluster, follow the steps for either [Creating VPC clusters](/docs/openshift?topic=openshift-cluster-create-vpc-gen2&interface=ui) or [Creating classic clusters](/docs/openshift?topic=openshift-cluster-create-classic).
...
The file sharing application saves files to the {{site.data.keyword.cos_short}} bucket. The relationship between files and users is stored as metadata in the {{site.data.keyword.cloudant_short_notm}} database. In this section, you'll create and configure these services.
...
4. Under **Service integrations (optional) / Monitoring & Activity tracking**, enable **Activity tracking** to have audit events persisted for analysis.
...
3. In the same tab under **Authentication Settings** under **Runtime Activity** enable capturing events in {{site.data.keyword.atracker_full_notm}}.
...
Now that the application and its services have been successfully deployed, you can review the security events generated by that process. All the events are centrally available in {{site.data.keyword.logs_full_notm}} instance.

1. From the [**Observability**](/observe/logging) dashboard, select the **Cloud Logs** tab and locate the {{site.data.keyword.logs_full_notm}} instance that is receiving the auditing events and click **Open dashboard**.
...
### Provision {{site.data.keyword.cis_short_notm}} and {{site.data.keyword.secrets-manager_short}} instances
...
* [{{site.data.keyword.containershort_notm}}](/docs/containers?topic=containers-iam-platform-access-roles) or the same for [{{site.data.keyword.openshiftshort}}](/docs/openshift?topic=openshift-iam-platform-access-roles).
