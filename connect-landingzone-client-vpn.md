...
The first thing that we need are Terraform modules for configuring a client-to-site VPC VPN Server, Security Groups, Secret Manager, and certificates. Fortunately, IBM wrote these modules already, and contributed them in open source in the `ibm-terraform-modules` repository.
...
- A Secret Manager instance (Optional - a reference to an existing Secret Manager instance can be passed through the existing_sm_instance_guid input variable)
...
1. Create a private catalog to hold your organization’s custom deployable architectures. This step is optional if your organization already has a private catalog for deployable architectures.
   a. Navigate to **Manage** > **Catalogs** > **Private Catalogs** in the {{site.data.keyword.cloud_notm}} console.
   b. Click **Create**.
   c. Give the catalog a name, for example “My Deployable Architectures”.
   d. Click **Create**.
...
3. Optional. Rather than using the private catalog UI, you can import the deployable architecture into your private catalog from the CLI. Edit the `--catalog` option to match the name of your private catalog. Also, make sure the `--target-version` and `--zipurl` match the Latest release version that is listed on the https://github.com/terraform-ibm-modules/terraform-ibm-client-to-site-vpn GitHub page. 
   ...
   b. When the import is complete, go back into your private catalog and verify that the new version is now listed in “Draft” state.
   c. The deployable architectures will not be visible to other users until you first Validate the deployable architecture to ensure that it works in your environment.

4. Select Validate from the **…** menu on the version.
   a. Validate the latest version of the deployable architecture in your private catalog. Validating this deployable architecture in the private catalog involves specifying input values that will be passed to Schematics, which then runs the Terraform plan and apply steps in the cloud to make sure that the deployable architecture actually works.
   b. Follow the steps to read the license file and select a schematics instance to run the DA with: 
   c. Enter the input values that are specific to your landing zone:
      i. Select an api key from your secret manager instance
      ii. vpc_id                             = "{VPC Id}"   (This field is found in the Optional inputs section of the UI)
      iii. region                             = "{region where the existing VPC is located}” (this field must be set to the region where the vpc referred by the vpc_id input is located)
      iv. resource_group             = "{landingzone_prefix}-management-rg"
      v. existing_sm_instance_guid = "{secretmgr_guid}"
      vi. existing_sm_instance_region = "{secretmgr_region}"
      vii. vpn_client_access_group_users = ["user1@domain.com",”user2@domain.com”,…]  (A list of users that are authorized to access the network that uses the new vpn connection. These users must be member of the {{site.data.keyword.cloud_notm}} account)
    d. Click **Validate**. After a few minutes, if correct inputs were provided, the validation should complete without errors. When the validation has completed successfully, the client-to-site VPN-related cloud resources are deployed in your account.
...
After the vpn server cloud resources have been deployed by using either the Manual or Deployable Architecture steps, you’ll need to setup OpenVPN client on devices that will access your landing zone.

1. Download the OpenVPN profile from the vpn server
   - By using the {{site.data.keyword.cloud_notm}} CLI: 

      The id of the vpnserver should be in the output of the terraform apply from verification. If not, open the vpnserver in {{site.data.keyword.cloud_notm}} console and download the `.ovpn` client profile to your local machine.
...
If your landing zone includes an OpenShift cluster, you can now test that you have access to the web console.
...
3. Click OpenShift Web Console button in the upper right corner. You should now be able to access your Red Hat OpenShift Web Console.
...
If you want to configure client certs on the VPN rather than using a one-time-passcode, there are instructions that you can follow in the [Managing VPN Server and Client Certifications section of the VPN Client to Site documentation](/docs/vpc?topic=vpc-client-to-site-authentication).
...
If you get an error when testing your vpn connectivity, such as the following: 
...
Automating the creation of Client-to-Site VPN connections to your secure landing zones is made straightforward using deployable architecture capabilities of {{site.data.keyword.cloud_notm}}.
